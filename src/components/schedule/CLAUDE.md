...
Grid semanal (LU-SA, horas 7:00-22:00, 1px = 1 minuto) de los grupos en `selectedGroups`, con fondo `bg-base-cream` y líneas de hora/día marcadas (`itam-dark/25`) para que se vean sobre el fondo de la app. Cada bloque usa el color de su materia (`coursePalette.ts`); si hay traslape con otro bloque seleccionado, se le agrega un borde punteado `#8C5E58` encima (no se repinta de rojo sólido, para no perder de vista de qué materia es). El bloque muestra clave + nombre de la materia (línea 1) y profesor + horario (línea 2), truncados con ellipsis; el `title` del div trae el detalle completo para hover cuando el bloque es muy chico.
